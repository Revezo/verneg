PlayerPositionRepository <- {
    ...
    function extractPosition(connection, playerName) {
        ...
        if (result) {
            ...
            if (row_assoc) {
                return Position(row_assoc["pos_x"], row_assoc["pos_y"], row_assoc["pos_z"])
            } else {
                return Position()
            }
        } else {
    ...
    function saveLastPositionQuery(playerName, position) {
        return format("INSERT INTO characters_positions(character_id, pos_x, pos_y, pos_z) " +
        "SELECT id, %d, %d, %d FROM characters WHERE `name`='%s' LIMIT 1 ", position.x, position.y, position.z, playerName) +
        format("ON DUPLICATE KEY UPDATE pos_x=%d, pos_y=%d, pos_z=%d", position.x, position.y, position.z)
    }
}